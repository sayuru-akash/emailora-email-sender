...
<template>
    ...
    <main class="mx-auto w-full max-w-3xl px-4 py-6 lg:px-8">
        <PageHeader
            ...
            subtitle="CSV/XLSX upload with duplicate handling"
        />
        <form
            class="space-y-4 rounded-lg border bg-card p-5"
            @submit.prevent="form.post('/imports/upload')"
        >
            <input
                type="file"
                accept=".csv,.txt,.xlsx"
            ...
            >
                ...
                <option value="add_to_list_tag">add to list/tag only</option>
                <option value="upsert">create or update</option></select
